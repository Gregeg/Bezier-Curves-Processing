void adjustControlPoints(int pi, Vector2D dv, boolean up, int mouseInd) {//pi is point index
  ...
  if (mouseInd == 0 && pi != 0 && !up) {
    ...
    dv = dv.scale(2);
    allPoints.get(pi-1)[allPoints.get(pi-1).length-2].setPos(allPoints.get(pi-1)[allPoints.get(pi-1).length-2].getPos(0).add(dv));
    ...
    dv = dv.scale(2);
    allPoints.get(pi+1)[1].setPos(allPoints.get(pi+1)[1].getPos(0).add(dv));
    if (allPoints.get(pi+1).length == 3)
