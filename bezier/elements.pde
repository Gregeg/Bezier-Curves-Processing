...
void drawBot(Vector2D pos, double rot) {
  ...
  fill(255, 0, 0, 50);
  if(simulation){
    image(botrot, -(float)botDim.x/2, -(float)botDim.y/2, (float)botDim.x, (float)botDim.y);
    if(skidding)
      rect(-(float)botDim.x/2, -(float)botDim.y/2, (float)botDim.x, (float)botDim.y);
  }else
    image(botrot, -botrot.width/2, -botrot.height/2, botrot.width, botrot.height);
